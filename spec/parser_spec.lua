...
describe("playlist parser", function()
  ...
  it("should parse a media playlist with discontinuity", function()
    local playlist = parser.parse_media_playlist(file.read("spec/samples/media_with_discontinuity.m3u8"))
    assert.are.same(#playlist.segments, 4)
    assert.are.same(playlist.segments[1].discontinuity, nil)
    assert.are.same(playlist.segments[2].discontinuity, true)
    assert.are.same(playlist.segments[3].discontinuity, nil)
    assert.are.same(playlist.segments[4].discontinuity, true)
  end)
